...
app.layout = html.Div(style={"display": "flex", "minHeight": "100vh", "backgroundColor": C["bg"], "fontFamily": "Arial, sans-serif"}, children=[
    ...
    html.Div(style={
        ...
        html.Label("Origin Airport", style={"color": C["muted"], "fontSize": "0.75rem", "letterSpacing": "1px", "textTransform": "uppercase"}),
        dcc.Dropdown(
            id="origin",
            options=[{"label": f"{k} — {v['name']}", "value": k} for k, v in AIRPORTS.items()],
            value="ORD",
            style={"backgroundColor": C["card"], "color": C["text"], "border": f"1px solid {C['border']}"},
            className="dark-dropdown"
        ),

        html.Label("Destination Airport", style={"color": C["muted"], "fontSize": "0.75rem", "letterSpacing": "1px", "textTransform": "uppercase"}),
        dcc.Dropdown(
            id="destination",
            options=[{"label": f"{k} — {v['name']}", "value": k} for k, v in AIRPORTS.items()],
            value="LHR",
        ...
        html.Label("Aircraft", style={"color": C["muted"], "fontSize": "0.75rem", "letterSpacing": "1px", "textTransform": "uppercase"}),
        dcc.Dropdown(
            id="aircraft",
        ...
        html.Label("Compare Aircraft", style={"color": C["muted"], "fontSize": "0.75rem", "letterSpacing": "1px", "textTransform": "uppercase"}),
        dcc.Dropdown(
            id="aircraft2",
        ...
        html.Label("Payload (kg)", style={"color": C["muted"], "fontSize": "0.75rem", "letterSpacing": "1px", "textTransform": "uppercase"}),
...
def update(n_clicks, origin, destination, aircraft, aircraft2, payload):
    if not origin or not destination or origin == destination:
        return html.P("Please select different origin and destination.", style={"color": C["red"]})
    ...
    fig1 = go.Figure(go.Scatter(x=altitudes/1000, y=ld_values, mode="lines", line=dict(color=C["blue"], width=2.5)))
    fig1.update_layout(title=dict(text="L/D Ratio vs Altitude", font=dict(color=C["blue"])), **chart_style)

    fig2 = go.Figure(go.Scatter(x=altitudes/1000, y=fuel_values, mode="lines", line=dict(color=C["red"], width=2.5)))
    fig2.update_layout(title=dict(text="Fuel Burn vs Altitude", font=dict(color=C["blue"])), **chart_style)

    fig3 = go.Figure(go.Scatter(x=altitudes/1000, y=co2_values, mode="lines", line=dict(color=C["green"], width=2.5)))
    fig3.update_layout(title=dict(text="CO2 Emissions vs Altitude", font=dict(color=C["blue"])), **chart_style)
    ...
    folium.PolyLine(result["waypoints"], color="#00bfff", weight=2.5, opacity=0.9).add_to(m)
    for code in [origin, destination]:
        ap = AIRPORTS[code]
        folium.CircleMarker([ap["lat"], ap["lon"]], radius=6, color="#00bfff",
            fill=True, fill_color="#00bfff", fill_opacity=1.0, popup=ap["name"]).add_to(m)
    ...
    if aircraft2 and aircraft2 != "none" and aircraft2 != aircraft:
        ...
        comparison = html.Div([
            html.H3("Aircraft Comparison", style={"color": C["blue"], "letterSpacing": "1px"}),
            html.Div(style={"display": "flex", "gap": "20px"}, children=[
                html.Div(style={"flex": "1", "display": "flex", "flexDirection": "column", "gap": "12px"}, children=[
                    html.P(aircraft, style={"color": C["text"], "fontWeight": "bold", "fontSize": "1rem"}),
                ...
                html.Div(style={"flex": "1", "display": "flex", "flexDirection": "column", "gap": "12px"}, children=[
                    html.P(aircraft2, style={"color": C["text"], "fontWeight": "bold", "fontSize": "1rem"}),
                    metric_card("Fuel Burned", f"{result2['fuel_burned_kg']:,} kg"),
                    metric_card("Flight Time", f"{result2['flight_time_hr']} hrs"),
                    metric_card("Cruise Altitude", f"{result2['cruise_altitude_ft']:,} ft"),
                    metric_card("L/D Ratio", str(result2['LD_ratio'])),
                    metric_card("CO2 Emissions", f"{result2['co2_tonnes']} t"),
                ]),
            ])
        ], style={"marginTop": "30px"})

    return html.Div(style={"display": "flex", "flexDirection": "column", "gap": "30px"}, children=[

        # Route title
        html.H2(f"{result['origin']} → {result['destination']}",
            style={"color": C["text"], "margin": "0", "fontSize": "1.6rem"}),
        ...
        # Map
        html.Div([
            html.H3("Route Map", style={"color": C["blue"], "letterSpacing": "1px", "margin": "0 0 16px 0"}),
        ...
        # Charts
        html.Div([
            html.H3("Performance vs Altitude", style={"color": C["blue"], "letterSpacing": "1px", "margin": "0 0 16px 0"}),
            html.Div(style={"display": "flex", "gap": "16px"}, children=[
                html.Div(dcc.Graph(figure=fig1, config={"displayModeBar": False}),
                    style={"flex": "1", "backgroundColor": C["card"], "borderRadius": "8px", "border": f"1px solid {C['border']}"}),
